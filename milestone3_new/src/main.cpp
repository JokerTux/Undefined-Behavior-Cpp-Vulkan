...
int main(int argc, char** argv){
    if(SDL_Init(SDL_INIT_VIDEO) != 0){
        ...
        return false;//return false (int main)
    }
    ...
    {   
        ...
        Mesh mesh = loadModel("src/assets/Meshes/girl OBJ.obj");
        Mesh mesh1 = loadModel("src/assets/Meshes/girl OBJ.obj");
        ...
        float size = glm::length(maxB - minB);
        
        std::cout << "Mesh center: " << center.x << " " << center.y << " " << center.z << std::endl;
        std::cout << "Mesh size:   " << size << std::endl;

        if(!create_mesh_buffers(&vkcontext, mesh)){
            std::cerr << "create_mesh_buffers failed" << std::endl;
            return false;
        }

        if(!create_ubo(&vkcontext)){
            std::cerr << "create_ubo failed" << std::endl;
            return false;
        }
        ...
        while(display_window.get_window_state()){
            ...
            float avg_dt = stats.avg_dt;
            //std::cout << "stats : " << stats.avg_dt << std::endl;
            ...
            float aspect = static_cast<float>(vkcontext.swapchainExtent.width) / static_cast<float>(vkcontext.swapchainExtent.height);
            update_ubo(&vkcontext, aspect, avg_dt);
        ...
        destroy_ubo(&vkcontext);
        destroy_mesh_buffers(&vkcontext);
    }
