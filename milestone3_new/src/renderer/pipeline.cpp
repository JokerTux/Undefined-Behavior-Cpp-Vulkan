...
static std::vector<char> read_b_shader(const std::string& file_path){
	...
	size_t file_size = static_cast<size_t>(file.tellg()); //might be useful later.
...
bool Graphics_pipeline::create_pipeline(VkContext* vkcontext){
	...
	auto vertShaderCode = read_b_shader("shaders/vert.spv");
	auto fragShaderCode = read_b_shader("shaders/frag.spv");
