...
bool Command_pool::record_command_buff(VkContext* vkcontext, uint32_t currentFrame, uint32_t imageIndex){
	...
	VkSubpassBeginInfo subpass_info{VK_STRUCTURE_TYPE_SUBPASS_BEGIN_INFO};
    subpass_info.contents = VK_SUBPASS_CONTENTS_INLINE;

	VkSubpassEndInfo subpass_end_info{VK_STRUCTURE_TYPE_SUBPASS_END_INFO};
    subpass_info.pNext = nullptr;

    
	VkBuffer vertexBuffers[] = {vkcontext->vertexBuffer};
    VkDeviceSize offsets[] = {0};
    
	vkCmdBeginRenderPass2(cmd, &renderPassInfo, &subpass_info);
	vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, vkcontext->graphicsPipeline);

	vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, vkcontext->pipelineLayout, 0, 1, &vkcontext->descriptorSet, 0, nullptr);
	
    vkCmdBindVertexBuffers(cmd, 0, 1, vertexBuffers, offsets);

    vkCmdBindIndexBuffer(cmd, vkcontext->indexBuffer, 0, VK_INDEX_TYPE_UINT32);

	vkCmdDrawIndexed(cmd, vkcontext->indexCount, 1, 0, 0, 0);
